...
class DocxShellGenerator:
    ...
    def generate(self) -> str:
        ...
        self._remove_trailing_page_break()
        self._set_document_properties()
        os.makedirs(os.path.dirname(self.output_path), exist_ok=True)
        self.doc.save(self.output_path)
        return self.output_path

    def _remove_trailing_page_break(self):
        """Remove only the final TFL separator so Word does not emit a blank last page."""
    ...
    def _add_tfl_shell(self, tfl):
        ...
        # Header block includes the single visible Heading 4 and bookmark.
        ...
        # Put the next TFL on a new page without creating a standalone break
        # paragraph, which can become an otherwise blank page when content is full.
        if doc.paragraphs:
    ...
    def _add_table_body(self, tfl):
        ...
        create_three_line_table(
            ...
            header_rows=tfl.effective_header_rows,
            column_alignments=tfl.column_alignments,
        )
    ...
    def _add_figure_body(self, tfl):
        ...
        if self.generate_figures and tfl.is_figure_generated:
            try:
                ...
                display_width = min(tfl.figure_width_inches, 6.0)
                if tfl.figure_height_inches > 4.5:
                    display_width *= 4.5 / tfl.figure_height_inches
                run_img.add_picture(buf, width=Inches(display_width))
    ...
    def _add_figure_shell_note(self):
        ...
        p_shell.paragraph_format.keep_with_next = True
        run = p_shell.add_run(
    ...
    def _add_listing_body(self, tfl):
        ...
        p.paragraph_format.keep_with_next = True
        sorting_note = tfl.sorting_note or "Sorted by site and subject ID."
        run = p.add_run(f"Note: {sorting_note}")
        run.font.size = Pt(config.FONT_SIZE_FOOTNOTE)
        run.font.italic = True
        run.font.name = config.FONT_NAME
