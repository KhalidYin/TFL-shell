    ...
    def _add_figure_body(self, tfl):
        ...
        if self.generate_figures and tfl.is_figure_generated:
            try:
                ...
                p.paragraph_format.keep_with_next = True
                run = p.add_run()
                run.add_picture(buf, width=Inches(tfl.figure_width_inches))
                p2 = doc.add_paragraph()
                p2.alignment = WD_ALIGN_PARAGRAPH.CENTER
                p2.paragraph_format.space_before = Pt(0)
                p2.paragraph_format.space_after = Pt(2)
                run2 = p2.add_run(f"[{tfl.display_label}: {tfl.figure_description}]")
                run2.font.size = Pt(9)
        ...
        if tfl.figure_description:
            run2 = p.add_run(tfl.figure_description)
            run2.font.size = Pt(9)
            run2.font.italic = True
            run2.font.name = config.FONT_NAME
