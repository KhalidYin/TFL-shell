...
class SwimmerFigure(ClinicalFigure):
    ...
    def build(self, data: dict) -> plt.Figure:
        ...
        for i, subj in enumerate(subjects):
            ...
            arm_color = TRT_COLORS_LIST[arm % len(TRT_COLORS_LIST)] if isinstance(arm, int) else TRT_COLORS.get(arm, "#999999")

            # Main treatment bar
            ax.barh(y, dur, height=0.6, color=arm_color, alpha=0.7,
                    edgecolor="white", linewidth=0.5)

            # Post-treatment follow-up bar (light gray)
            if pd_time is not None and pd_time > dur:
                ax.barh(y, pd_time - dur, left=dur, height=0.6,
                        color="lightgray", alpha=0.5, edgecolor="white", linewidth=0.5)
            ...
            if resp_time is not None and resp_type:
                ...
                ax.plot(resp_time, y, marker=marker, color="gold",
            ...
            # First response marker (circle) if resp_time but no specific type
                ...
                ax.scatter(resp_time, y, marker="o", facecolor="white",
                           edgecolor="#333333", s=30, zorder=5, linewidth=1)

            # PD marker (triangle)
            if pd_time is not None:
                ax.plot(pd_time, y, marker="v", color="black",
        ...
        legend_items.extend([
            ...
            Line2D([0], [0], marker="*", color="gold", label="CR",
                   markerfacecolor="gold", markersize=10, linewidth=0, markeredgecolor="black"),
            Line2D([0], [0], marker="D", color="gold", label="PR",
                   markerfacecolor="gold", markersize=8, linewidth=0, markeredgecolor="black"),
            Line2D([0], [0], marker="v", color="black", label="PD",
                   markerfacecolor="black", markersize=8, linewidth=0),
            Line2D([0], [0], marker="+", color="#333333", markersize=10,
                   linewidth=0, label="Ongoing at Cutoff"),
        ])
        ax.legend(handles=legend_items, loc="lower right", fontsize=7,
                  frameon=True, facecolor="white", edgecolor="#DDDDDD", ncol=2)
    ...
    def _mock_swimmer_data(n_subjects=14, seed=42):
        ...
        for i in range(n_subjects):
            ...
            pd_t = dur + rng.exponential(4) + 0.5 if has_pd and rng.random() < 0.5 else (dur + 0.1 if has_pd else None)
            ongoing = not has_pd and rng.random() < 0.3
            subjects.append({
                "id": f"Subj {i+1:03d}",
                "arm": arm, "duration": dur, "bor": bor,
                "resp_time": ttr, "resp_type": bor if bor in ("CR", "PR") else None,
                "pd_time": pd_t, "ongoing": ongoing,
            })
