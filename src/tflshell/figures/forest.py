    ...
    def build(self, data: dict) -> plt.Figure:
        ...
        for i, row in enumerate(rows):
            ...
            if is_overall:
                ...
                ax.scatter(hr, y, color=color, s=50, zorder=3, marker="o", linewidth=1)

            # HR/CI text
            hr_text = f"{hr:.2f} ({ci_low:.2f}, {ci_high:.2f})"
            if xscale == "log":
                ax.text(
                    2.8, y, hr_text, fontsize=7, fontfamily="monospace", verticalalignment="center"
                )
            else:
                ax.text(
                    ax.get_xlim()[1] * 1.02,
